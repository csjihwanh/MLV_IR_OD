# original code reference: https://github.com/ax2mazhr/FLIR2YOLO
...
if __name__ == '__main__':
    ...
    parser.add_argument("--base_dir", help='')

    args = parser.parse_args()

    labels_dir = os.path.join(args.base_dir, "labels")
    ...
    os.mkdir(labels_dir)
    count = 0
    labels = ['person', 'car', 'truck', 'bus', 'bike', 'motor', 'other vehicle', 'dog']
    # ['person','bike','car','motor','bus','train','truck','light','dog','scooter','other vehicle']
    # hscai label : [Person(0), Car(1), Truck(2), Bus(3), Bicycle(4) Bike(5), Extra_vehicle(6), Dog(7)​]

    with open(f'{args.base_dir}/index.json') as f:
        data = json.load(f)
        frames = data['frames']
        width = 640.0
        height = 512.0
        ...
        for frame in frames:
            ...
            count += 1
            img = cv2.imread(f"{args.base_dir}/data/{image_name}")
            converted_results = []  
            ...
            #cv2.waitKey(1)
            print(f'{args.image_directory}/{image_name}')
            file = open(f"{args.base_dir}/labels/" + str(image_name)[:-4] + '.txt', 'w+')
            file.write('\n'.join('%d %.6f %.6f %.6f %.6f' % res for res in converted_results))
            file.close()
        file = open(f'{args.base_dir}/all_image_names.txt', 'w+')
